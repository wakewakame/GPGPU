...
namespace gpu
{
	...
	*/
	...
	private:
		...
		std::vector<GLuint> SSBO_IN; // 入力SSBOオブジェクトのID
		std::vector<GLuint> SSBO_OUT; // 出力SSBOオブジェクトのID
		...
		*/
		...
		void deleteShader() {
			...
			if (SSBO_IN.size() != 0) {
				glDeleteBuffers(SSBO_IN.size(), SSBO_IN.data());
			}
			if (SSBO_OUT.size() != 0) {
				glDeleteBuffers(SSBO_OUT.size(), SSBO_OUT.data());
			}
		}
